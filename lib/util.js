...
function isArtifact(url,options) {
    logger.debug(`isArtifact($url)?`);
    const base = options['base'] ? options['base'] : `http://${options['host']}:${options['port']}`;
    const public = options['public'];
    const filePath = url.substring(base.length + 1);
    
    if (! fs.existsSync(`${public}/${filePath}.meta`)) {
        logger.debug(`no such file ${public}/${filePath}.meta`);
        return false;
    }

    const json = parseAsJSON(`${public}/${filePath}.meta`);

    if (json === null)  {
        logger.debug(`no json at ${public}/${filePath}`);
        return false;
    }

    return json['X-Artifact'];
}
