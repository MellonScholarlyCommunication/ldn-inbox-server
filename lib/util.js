...
function getLogger() {
    ...
    if (process.env.LOG4JS || process.env.LOG_FORMAT) {
        log4js.configure({
            appenders: {
            stderr: { type: 'stderr',
...
// Substitute ${VAR} / ${VAR:-default} from process.env. Unset with no default is
// left literal so a misconfigured value is visible (greppable) downstream.
function interpolateEnv(str) {
    return str.replace(/\$\{(\w+)(?::-([^}]*))?\}/g, (m, name, fallback) => {
...
function parseConfig(path) {
    try {
        const raw = interpolateEnv(fs.readFileSync(path, {encoding: 'utf-8'}));
...
function moveTo(path,destination,reason) {
    ...
    switch (destination) {
        ...
        case '@error':
            ...
            break;
        case '@outbox':
            destPath = process.env.LDN_SERVER_OUTBOX_PATH ?? './outbox';
            break;
    }
    ...
    // capture the failure reason next to the quarantined notification
    if (reason !== undefined && reason !== null) {
        try {
            ...
            fs.writeFileSync(fsPath.resolve(newPath + '.err'), `${new Date().toISOString()}\t${text}\n`);
        }
        catch (e) {
            logger.error(`failed to write error sidecar for ${newPath}`);
            logger.error(e);
        }
    }
}
...
function parseLocalResource(url,options) {
    ...
    // Refuse paths that escape the public root (path traversal)
    const publicRoot = fsPath.resolve(public);
    const resolved = fsPath.resolve(filePath);
    if (resolved !== publicRoot && ! resolved.startsWith(publicRoot + fsPath.sep)) {
        logger.error(`refusing ${url} : resolves outside ${public}`);
        return null;
    }
